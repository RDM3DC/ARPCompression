...
# Clean arithmetic coder (binary stream), adaptive order-0 model.
...
class BitWriter:
    ...
    def write_pending(self, bit: int, count: int):
        for _ in range(count):
            self.write_bit(bit ^ 1)  # E3 underflow flips
...
class Model:
    def __init__(self, n: int, max_total: int = 1<<15):
        self.n = n
        self.freq = [1]*n   # Laplace smoothing
        self.cum  = [0]*(n+1)
        self.total = 0
    ...
    def update(self, sym: int):
        ...
        if self.total >= self.max_total:
            # halve frequencies (keep nonzero)
            for i in range(self.n):
...
class Encoder:
    ...
    def finish(self):
        self.pending += 1
        if self.low < Q1:
            self.bw.write_bit(0); self.bw.write_pending(0, self.pending)
        else:
            self.bw.write_bit(1); self.bw.write_pending(1, self.pending)
...
class Decoder:
    ...
    def decode(self, model: Model) -> int:
        ...
        # find symbol (linear scan OK for tiny alphabets)
        sym = 0
